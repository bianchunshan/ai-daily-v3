...
- **每小时富化上限 / 保留条数**:`enrich_news.py` 顶部 `CAP`(默认 20)、`KEEP`(默认 40)。
...
- 仓库 Secrets:`QWEN_KEY`、`VERCEL_TOKEN`、`VERCEL_ORG_ID`、`VERCEL_PROJECT_ID`。
...
- 新闻"X小时前"是抓取时的静态字符串,不会自动刷新(随滚动淘汰)。
- 新浪行情非官方接口,稳定性不保证。
